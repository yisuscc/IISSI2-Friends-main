...
async function handleSubmitPhoto(event) {
	...
	if (currentPhoto === null) { // Creating a new photo
		...
		try { 	let resp = await picturesAPI_auto.create(formData); // API REST para insertar nueva foto
				...
				window.location.href = `editPhoto.html?pictureId=${newId}`; // Ver la foto añadida
		...
		try {	await picturesAPI_auto.update(formData, pictureId); // API REST para actualizar la foto
				window.location.href = `perfilPropio.html?pictureId=${pictureId}`;
		} catch (err) { messageRenderer.showErrorAsAlert(err.response.data.message); }
	}
}
